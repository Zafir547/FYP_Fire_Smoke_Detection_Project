...
# # ── Suppress OpenCV camera warnings (cloud server ke liye) ──
...
#   .metric-card.accuracy { border: 1px solid rgba(0,255,136,0.4); box-shadow: 0 0 20px rgba(0,255,136,0.1); }
#   .metric-card.frames { border: 1px solid rgba(180,100,255,0.4); box-shadow: 0 0 20px rgba(180,100,255,0.1); }
#   .metric-label { font-family: 'Rajdhani', sans-serif; font-size: 0.75rem; letter-spacing: 3px; text-transform: uppercase; opacity: 0.6; margin-bottom: 8px; }
#   .metric-value { font-family: 'Orbitron', monospace; font-size: 2.2rem; font-weight: 700; line-height: 1; }
#   .metric-card.fire .metric-value { color: #ff5722; }
#   .metric-card.smoke .metric-value { color: #64a0ff; }
#   .metric-card.accuracy .metric-value { color: #00ff88; }
#   .metric-card.frames .metric-value { color: #b464ff; }
...
#   [data-testid="stSidebar"] { background: rgba(10,12,16,0.98) !important; border-right: 1px solid rgba(255,80,0,0.15) !important; }
#   .stButton > button {
#     background: linear-gradient(135deg, #ff3c00, #ff6b35) !important;
#     color: white !important;
#     border: none !important;
...
#     box-shadow: 0 0 20px rgba(255,60,0,0.3) !important;
#   }
#   .stButton > button:hover { transform: translateY(-2px) !important; box-shadow: 0 0 30px rgba(255,60,0,0.5) !important; }
...
#     now = datetime.now()
#     st.markdown(f"""
#     <div style='font-family: Orbitron, monospace; font-size:0.65rem; letter-spacing:2px; color:rgba(255,150,80,0.6); text-align:center;'>
#       SYS TIME<br>
#       <span style='font-size:1rem; color:#ff6b35;'>{now.strftime('%H:%M:%S')}</span><br>
#       <span style='opacity:0.5;'>{now.strftime('%Y-%m-%d')}</span>
#     </div>
...
#     # CAMERA MODE
#     elif option == "Camera":
#         st.info("📷 **Camera only works locally** (on your own laptop/desktop with webcam).\n\n"
#                 "On cloud servers there is no camera attached. Use **Image** or **Video** instead.")
...
#             cap = cv2.VideoCapture(0)
#             if not cap.isOpened():
#                 st.error("❌ No camera found on this server.")
#                 st.session_state.camera_active = False
#                 st.rerun()
#             else:
#                 ret, frame = cap.read()
...
with left_col:
    ...
    #     st.info("📷 Browser camera allow karein. Agar blank screen aaye to Image mode se test karein pehle.")
    ...
    #     def video_frame_callback(frame):
    #         try:
    #             # Raw frame from browser
    #             img = frame.to_ndarray(format="bgr24")

    #             # ←←←← TEST MODE: Sirf original frame wapas bhejo (no processing)
    #             # processed_frame, det = process_frame(img)   # Comment this line temporarily

    #             # TEMPORARY: No detection, just return original frame (black screen fix test)
    #             processed_frame = img.copy()   # ← Yeh line important hai test ke liye

    #             # Dummy detection for testing
    #             det = {'fire': False, 'smoke': False}

    #             # Update counters (light weight)
    #             ts = datetime.now().strftime("%H:%M:%S")
    #             if det.get('fire'):
    #                 st.session_state.fire += 1
    #                 st.session_state.detection_log.append({"time": ts, "type": "🔥 Fire", "frame": st.session_state.frames})
    ...
    #             st.error(f"Camera Error: {str(e)[:100]}...")
    #             # Fallback: return original frame
    #             return frame.to_ndarray(format="bgr24")

    #     webrtc_ctx = webrtc_streamer(
    #         key="aegis_camera_test",
...
# Good Working
    ...
    #     st.info("📷 Camera ko allow karein. Fire/Smoke detection real-time mein hoga.")
    ...
    #             # Process with your detection model
    #             processed_frame, det = process_frame(img)
    ...
    #         except Exception as e:
    #             # Agar koi error aaye to original frame return kar do (crash na ho)
    #             st.error(f"Callback Error: {str(e)}")
    #             return frame.to_ndarray(format="bgr24")

    #     # WebRTC Streamer
    #     webrtc_ctx = webrtc_streamer(
    #         key="aegis_live_camera",
    #         mode=WebRtcMode.SENDRECV,
    #         rtc_configuration=RTC_CONFIGURATION,
    #         video_frame_callback=video_frame_callback,
    #         media_stream_constraints={
    ...
    #     if webrtc_ctx.state.playing:
    #         st.success("✅ Camera Connected & Detection Running")
    #         time.sleep(0.4)
    #         st.rerun()
